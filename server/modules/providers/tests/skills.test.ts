 ...
 */
test('providerSkillsService adds global skills for claude, codex, cursor, and kiro', { concurrency: false }, async () => {
  ...
  try {
    ...
    const createdKiroSkills = await providerSkillsService.addProviderSkills('kiro', {
      entries: [
        {
          directoryName: 'kiro-global-dir',
          content: '---\nname: kiro-global\ndescription: Kiro global skill\n---\n\nKiro body.\n',
        },
      ],
    });
    const createdKiroSkill = createdKiroSkills[0];
    assert.ok(createdKiroSkill);
    assert.equal(createdKiroSkill.command, '/kiro-global');
    assert.equal(
      createdKiroSkill.sourcePath.endsWith(path.join('.kiro', 'skills', 'kiro-global-dir', 'SKILL.md')),
      true,
    );
    ...
    const listedKiroSkills = await providerSkillsService.listProviderSkills('kiro');
    assert.equal(listedKiroSkills.some((skill) => skill.name === 'kiro-global'), true);
