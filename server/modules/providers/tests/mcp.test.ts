...
/**
 * This test covers Kiro MCP support for user/project stdio config files and
 * rejects remote transports that the Kiro CLI cannot consume.
 */
test('providerMcpService handles Kiro MCP JSON config and capability validation', { concurrency: false }, async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'llm-mcp-kiro-'));
  const workspacePath = path.join(tempRoot, 'workspace');
  await fs.mkdir(workspacePath, { recursive: true });

  const restoreHomeDir = patchHomeDir(tempRoot);
  try {
    await providerMcpService.upsertProviderMcpServer('kiro', {
      name: 'kiro-project-stdio',
      scope: 'project',
      transport: 'stdio',
      command: 'node',
      args: ['server.js'],
    ...
    const projectServer = (projectConfig.mcpServers as Record<string, unknown>)[
      'kiro-project-stdio'
    ] as Record<string, unknown>;
    assert.equal(projectServer.command, 'node');
    assert.deepEqual(projectServer.args, ['server.js']);
    assert.deepEqual(projectServer.env, { API_KEY: 'value' });

    await assert.rejects(
      providerMcpService.upsertProviderMcpServer('kiro', {
        name: 'kiro-project-http',
        scope: 'project',
        transport: 'http',
        url: 'https://example.com/mcp',
        workspacePath,
...
/**
 * This test covers the global MCP adder requirement: only http/stdio are accepted
 * and every registered provider returns an explicit success or capability error.
 */
test('providerMcpService global adder attempts all providers and rejects unsupported transports', { concurrency: false }, async () => {
  ...
  try {
    ...
    assert.equal(
      globalResult.find((entry) => entry.provider === 'kiro')?.created,
      false,
    );
    assert.ok(
      globalResult
