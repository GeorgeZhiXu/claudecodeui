...
export function createAgentRouter(dependencies: AgentRouterDependencies): express.Router {
  ...
  const queryKiro = dependencies.queryKiro;
   ...
   *
   * @param {string} provider - (Optional) AI provider to use. Options: 'claude' | 'cursor' | 'codex' | 'kiro' | 'opencode'
   *                           Default: 'claude'
   *
   ...
   *   - provider must be 'claude', 'cursor', 'codex', 'kiro', or 'opencode'
  ...
  router.post('/', validateExternalApiKey, async (req, res) => {
    ...
    if (!['claude', 'cursor', 'codex', 'kiro', 'opencode'].includes(provider)) {
      return res.status(400).json({ error: 'provider must be "claude", "cursor", "codex", "kiro", or "opencode"' });
    }
    ...
    try {
      ...
      const kiroModels = await providerModelsService.getProviderModels('kiro');
      ...
      if (provider === 'claude') {
        ...
        await queryKiro(message.trim(), {
          projectPath: finalProjectPath,
          cwd: finalProjectPath,
          sessionId: sessionId || null,
          model: model || kiroModels.DEFAULT,
          permissionMode: 'bypassPermissions'
        }, writer);
