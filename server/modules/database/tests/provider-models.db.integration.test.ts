...
test('migrations preserve custom models while adding Kiro to the provider constraint', async () => {
  const previousDatabasePath = process.env.DATABASE_PATH;
  ...
  try {
    ...
    db.prepare(`
      ...
      VALUES ('codex', 'existing-model', 'Existing Model', 3)
    `).run();

    runMigrations(db);

    assert.equal(
      providerModelsDb.findCustomProviderModelByModelId('codex', 'existing-model')?.sortOrder,
      3,
    );
    const kiroModel = providerModelsDb.createCustomProviderModel('kiro', {
      model: 'Kiro Custom',
      id: 'kiro-custom',
    });
    assert.equal(kiroModel.provider, 'kiro');
  } finally {
    closeConnection();
    if (previousDatabasePath === undefined) {
      delete process.env.DATABASE_PATH;
