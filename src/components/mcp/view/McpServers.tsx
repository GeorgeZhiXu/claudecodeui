...
export default function McpServers({ selectedProvider, currentProjects }: McpServersProps) {
  ...
  const globalAddDescription = 'Writes one stdio server to Claude, Cursor, Codex, Kiro, and OpenCode, or one HTTP server to every listed provider except Kiro.';
  const providerAddDescription = `${providerButtonLabel} only changes ${providerName}.`;
  const globalModalDescription = 'Adds this MCP server to each provider that supports its transport. '
    + 'Stdio works with Claude, Cursor, Codex, Kiro, and OpenCode; HTTP works with Claude, Cursor, Codex, and OpenCode.';
